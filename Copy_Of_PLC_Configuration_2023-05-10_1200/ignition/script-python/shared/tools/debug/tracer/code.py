...
try:
	IGNITION_VERSION = system.tag.readBlocking('[System]Client/System/FPMIVersion')[0].value
except:
	IGNITION_VERSION = '8.x.x' # placeholder until I find a better way - ARG
...
class Tracer(object):
	...
	def shutdown(self):
		...
		if self.tag_path:
			system.tag.writeBlocking(self.tag_path, 'Shutting down...')
			# disable the tag now that the tracer is irrecoverably offline
			system.tag.writeBlocking(self.tag_path + '.enabled', False)
		self.monitoring = False
		self._cursor_stack = tuple()
		try:
	...
	def _init_control_tag(self, control_tag):
		...
		self.tag_acked = True
		system.tag.writeBlocking(self.tag_path, '')
...
#				)
				...
				if self.tag_path:
					if not self.tag_acked:
						if isinstance(result, Exception):
							system.tag.writeBlocking(self.tag_path, 'Exception "%r" for command "%s"' % (result, command))
						elif isinstance(result, (list, tuple, dict)):
							system.tag.writeBlocking(self.tag_path, system.util.jsonEncode(result))
						elif result is None or result == '':
							system.tag.writeBlocking(self.tag_path, 'Done.')						
						else:
							system.tag.writeBlocking(self.tag_path, str(result))
	...
	def _await_command(self):
		if not self._pending_commands:
			...
			if self.tag_path:
				tag_command = system.tag.readBlocking(self.tag_path)[0].value
				if tag_command:
					if self.tag_acked:
